...
#include "CircularSegmentCutUserObject.h"

template <>
InputParameters
validParams<CircularSegmentCutUserObject>()
{
...
CircularSegmentCutUserObject::CircularSegmentCutUserObject(const InputParameters & parameters)
  : GeometricCut2DUserObject(parameters), _cut_data(getParam<std::vector<Real>>("cut_data"))
{
  ...
  if (_cut_data.size() != cut_data_len)
    mooseError("Length of CircularSegmentCutUserObject cut_data must be 3");
...
CircularSegmentCutUserObject::getCrackFrontPoints(unsigned int /*num_crack_front_points*/) const
{
