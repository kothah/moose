...
#include "XFEMSingleVariableEqConstraint.h"
...
registerMooseObject("XFEMApp", XFEMSingleVariableEqConstraint);

template <>
InputParameters
validParams<XFEMSingleVariableEqConstraint>()
{
...
XFEMSingleVariableEqConstraint::XFEMSingleVariableEqConstraint(const InputParameters & parameters)
    ...
    _use_penalty(getParam<bool>("use_penalty"))
{
  ...
  if (_xfem == nullptr)
    mooseError("Problem casting to XFEM in XFEMSingleVariableEqConstraint");
  ...
  if (dynamic_cast<const GeometricCutUserObject *>(uo) == nullptr)
    mooseError("UserObject casting to GeometricCutUserObject in XFEMSingleVariableEqConstraint");
...
XFEMSingleVariableEqConstraint::~XFEMSingleVariableEqConstraint() {}

void
XFEMSingleVariableEqConstraint::reinitConstraintQuadrature(
...
Real
XFEMSingleVariableEqConstraint::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0;
    ...
    switch (type)
    {
      ...
      case Moose::Neighbor:
        if (!_use_penalty)
        {
          ...
          r -=
              (_u[_qp] - _u_neighbor[_qp]) * 0.5 * _grad_test_neighbor[_i][_qp] * _interface_normal;
        }
...
Real
XFEMSingleVariableEqConstraint::computeQpJacobian(Moose::DGJacobianType type)
{
  Real r = 0;
  if ((_fe_problem.time() >= _time_from) && (_fe_problem.time() <= _time_to))
  {
