...
#include "XFEMDispBimaterialConstraint.h"
...
registerMooseObject("XFEMApp", XFEMDispBimaterialConstraint);

template <>
InputParameters
validParams<XFEMDispBimaterialConstraint>()
{
...
XFEMDispBimaterialConstraint::XFEMDispBimaterialConstraint(const InputParameters & parameters)
    ...
    _use_positive_property(false)
{
  ...
  if (_xfem == nullptr)
    mooseError("Problem casting to XFEM in XFEMDispBimaterialConstraint");
  ...
  if (dynamic_cast<const GeometricCutUserObject *>(uo) == nullptr)
    mooseError("UserObject casting to GeometricCutUserObject in XFEMDispBimaterialConstraint");
...
XFEMDispBimaterialConstraint::~XFEMDispBimaterialConstraint() {}

void
XFEMDispBimaterialConstraint::reinitConstraintQuadrature(const ElementPairInfo & element_pair_info)
{
  ...
  if ((_fe_problem.time() >= _time_from) && (_fe_problem.time() <= _time_to))
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementNeighbor:
        if (((_u[_qp] - _u_neighbor[_qp]) * norm_comp) >= 0)
        {
          r -= 0.5 * (phi_contact_pressure_neighbor * _elem_neighbor_vol_frac) *
               (_test[_i][_qp] * norm_comp);
          r +=
              (_phi_neighbor[_j][_qp] * norm_comp) * (0.5 * test_contact_pressure * _elem_vol_frac);
      ...
      case Moose::NeighborElement:
        ...
        {
          r +=
              0.5 * (phi_contact_pressure * _elem_vol_frac) * (_test_neighbor[_i][_qp] * norm_comp);
